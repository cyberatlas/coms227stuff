...
public abstract class GraphMap implements Updatable, Iterable<Cell> {
	...
	public GraphMap() {
		try {
			...
			throw new RuntimeException("Unable to create a default color generator.");
		}
		...
		for (String line : descriptor) {
			...
			for (int x = 0; x < cellRow.length; x++) {
				...
				try {
					...
					System.out.println("Unable to create an instance for " + clazz.getName());
				}
	...
	public void initialize() {
		...
		try {
			cells[cells[0].length / 2][cells.length / 2].setState(snake.newInstance());
		} catch (Exception e) {
	...
	/**
	 * Select the closest cell to the x and y pixel coordinates. Normally this
	 * will be the cell under the mouse cursor, unless the mouse has moved out
	...
	public void createFood() {
		...
		do {
			...
			Cell randomCell = cells[r.nextInt(cells.length)][r.nextInt(cells[0].length)];
			if (randomCell.getState() == null) {
				...
				randomCell = cells[r.nextInt(cells.length)][r.nextInt(cells[0].length)];
				if (randomCell.getState() == null) {
					try {
	...
	/**
	 * Recalculates all mouse distances, updates all cells, and creates food, if
	 * the previous food has been "eaten".
	 *
	...
	/**
	 * Gets the width of the window in pixels for rendering, including the
	 * border area.
	 *
	...
	/**
	 * Gets the height of the window in pix els for rendering, including the
	 * border area.
	 *
	...
	/**
	 * Get the column and row indices for the cell closest to a given pixel (x,
	 * y) coordinate, returned as a Point object in which x is the column and y
	 * is the row.
	 *
	 * @param x
