...
public class Cell implements Updatable {
	 ...
	 *
	 * @return array of adjacent cells to this one
	 */
	 ...
	 *
	 * @return mouse distance for this cell
	 */
	...
	/**
	 * Updates the mouse distance for cells near this one, assuming that the
	 * mouse is on this cell and using MAX_MOUSE_DISTANCE for this cell. For
	 * example, if one could get to a neighboring cell from this one by moving
	 * ten cells, then that cell's mouse distance will be set to to
	 * MAX_MOUSE_DISTANCE minus ten. Cells more than MAX_MOUSE_DISTANCE from
	 * this one will not be affected (will retain the default mouse distance of
	 * 0). Cells with an impassable state always have a mouse distance of 0.
	 */
	...
	public Cell getRandomOpen() {
		...
		Cell highCell = null;
		for (Cell neighbor : neighbors) {
			if (neighbor.getState() == null || !state.isPassable() && neighbor.getState().isPassable()) {
				double chance = random.nextDouble();
				if (chance > highest) {
					highCell = neighbor;
					highest = chance;
	...
	/**
	 * Sets the current state of this cell. NOTE: when an existing state is
	 * being moved from one cell to an adjacent cell, use the method
	 * <code>moveState</code>.
	 *
	 * @param state
