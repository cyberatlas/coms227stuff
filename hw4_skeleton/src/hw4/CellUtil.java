...
public class CellUtil {
	...
	public static void calculateMouseDistance(Cell cell, int distance) {
		...
		if (distance <= 0)
			return;
		// if the cell is null or the cell is of a passable state, then set the
		// mouse distance to the distance passed in
		if (cell.getState() == null || cell.getState().isPassable()) {
			cell.setMouseDistance(distance);
		}
		// loop through the array and if the distance is less than the
		// mousedisance, call the method again to calculate the distance for the
		// rest of the cells
