...
public class Config {
	...
	/**
	 * - The default color generator to use if a configuration cannot load a
	 * generator.
	 */
	...
	static {
		...
		for (int i = 0; i < FOOD_COLORS.length; i++) {
			FOOD_COLORS[i] = new Color(1.0f, 0.0f, 1.0f, (i + 1.0f) / FOOD_COLORS.length);
		}

		SNAKE_COLORS = new Color[MAX_SNAKE_TIMER];
		for (int i = 0; i < SNAKE_COLORS.length; i++) {
			SNAKE_COLORS[i] = new Color(1.0f, 0.0f, 0.0f, (i + 1.0f) / SNAKE_COLORS.length);
		}
		...
		for (File file : packag.listFiles()) {
			...
			try {
				...
				Class<State> clazz = (Class<State>) Class
						.forName("hw4." + fileName.substring(0, fileName.length() - 5));
				State s;
				s = clazz.newInstance();

				STATES.put(s.toChar(), clazz);
			} catch (ClassNotFoundException e) {
				System.out.println("No class was found for the file " + fileName + ".");
